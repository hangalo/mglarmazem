...
public class SaidaArmazemDAO {
    ...
    private static final String LISTAR_TUDO = "SELECT s.id_saida_armazem, s.data_saida_armazem, s.id_sector, s.quantidade_saida_armazem, s.id_produto, p.descricao_produto, e.preco_produto, sec.descricao_sector FROM saida_armazem s INNER JOIN produto p ON s.id_produto = p.id_produto INNER JOIN entrada_armazem e ON s.id_produto = e.id_produto INNER JOIN sector sec ON s.id_sector = sec.id_sector ORDER BY s.data_saida_armazem DESC";
    ...
    public boolean registrarSaida(SaidaArmazem saida) {
        ...
        try {
            ...
            if (conn != null) {
                try { conn.rollback(); } catch (SQLException ex) { LOGGER.log(Level.SEVERE, "Erro ao fazer rollback", ex); }
            }
    ...
    public List<EntradaArmazem> pesquisarSaidasPorProduto(java.util.Date inicio, java.util.Date fim) throws SQLException {
        ...
        try (Connection conn = ConnectionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(PESQUISAR_SAIDAS_PRODUTO)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    EntradaArmazem saida = new EntradaArmazem(rs.getString("descricao_prod"), rs.getInt("total_quantidade"), rs.getBigDecimal("total_valor"));
                    lista.add(saida);
                }
            }
        }
    ...
    public List<SaidaArmazem> listarTudo() throws SQLException {
        ...
        try (Connection conn = ConnectionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(LISTAR_TUDO); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                SaidaArmazem s = new SaidaArmazem(); s.setIdSaidaArmazem(rs.getInt("id_saida_armazem")); s.setIdProduto(rs.getInt("id_produto")); s.setIdSector(rs.getInt("id_sector")); s.setQuantidadeSaidaArmazem(rs.getInt("quantidade_saida_armazem")); s.setDataSaidaArmazem(rs.getDate("data_saida_armazem"));
                skylink.armazem.modelo.Produto p = new skylink.armazem.modelo.Produto(); p.setDescricaoProduto(rs.getString("descricao_produto")); s.getEntradaArmazem().setProduto(p); s.getEntradaArmazem().setPrecoProduto(rs.getDouble("preco_produto")); s.getSector().setDescricaoSector(rs.getString("descricao_sector"));
                lista.add(s);
            }
        } return lista;
    }
